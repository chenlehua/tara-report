...
输入数据格式说明：
1. cover_data: 封面信息 (JSON)
2. definitions_data: 相关定义 (JSON)
3. assets_data: 资产列表 (JSON)
4. attack_trees_data: 攻击树 (JSON)
5. tara_results_data: TARA分析结果 (JSON)
"""
...
def create_definitions_sheet(wb: Workbook, data: Dict[str, Any]) -> None:
    ...
    输入数据格式:
    {
        ...
        "functional_description": "车载信息娱乐系统(In-Vehicle Infotainment, IVI)是一种集成多媒体娱乐、导航...",
...
def create_attack_trees_sheet(wb: Workbook, data: Dict[str, Any]) -> None:
    ...
    输入数据格式:
    {
        ...
        "attack_trees": [
            {
                "title": "攻击树1 Attack Tree 1: 远程入侵IVI系统 Remote Compromise of IVI",
                "image": "/path/to/attack_tree_1.png"
            },
            {
                "title": "攻击树2 Attack Tree 2: 物理接口攻击 Physical Interface Attack",
                "image": "/path/to/attack_tree_2.png"
            }
        ]
    }
    """
...
def create_tara_results_sheet(wb: Workbook, data: Dict[str, Any]) -> None:
    ...
    输入数据格式:
    {
        "title": "MY25 EV平台中控主机_TARA分析结果 TARA Analysis Results",
        "results": [
            {
                "asset_id": "P001",
                "asset_name": "车载多媒体",
                ...
                "stride_model": "S欺骗",
                "threat_scenario": "黑客仿冒、篡改SOC模块...",
                "attack_path": "1.攻击者锁定待攻击车辆...",
                "wp29_mapping": "4.1\n5.1\n6.1",
                "attack_vector": "本地",
                "attack_complexity": "低",
...
def generate_tara_excel(
    ...
    参数:
        output_path: 输出文件路径
        cover_data: 封面数据 (JSON格式)
        definitions_data: 相关定义数据 (JSON格式)
        assets_data: 资产列表数据 (JSON格式)
        attack_trees_data: 攻击树数据 (JSON格式)
        tara_results_data: TARA分析结果数据 (JSON格式)
...
if __name__ == "__main__":
    ...
    sample_data = {
        ...
        "definitions": {
            "title": "MY25 EV平台中控主机 TARA分析报告 - 相关定义",
            "functional_description": "车载信息娱乐系统(In-Vehicle Infotainment, IVI)是一种集成多媒体娱乐、导航、蓝牙通信、车辆控制等功能的智能车载终端系统。",
            "assumptions": [
                {"id": "ASM-01", "description": "IVI系统运行在独立的电子控制单元上"},
                {"id": "ASM-02", "description": "IVI系统与车身域控制器通过CAN网关进行通信"}
            ],
            ...
            "assets": [
                {
                    ...
                    "remarks": "主处理器芯片", "authenticity": True, "availability": True
                }
            ]
        },
        "attack_trees": {
    ...
    generate_tara_excel_from_json("tara_report_sample.xlsx", sample_data)
    print("示例报告已生成: tara_report_sample.xlsx")
